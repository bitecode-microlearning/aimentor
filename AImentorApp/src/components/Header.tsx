    ...
    <header className="w-full bg-[#1376C8] text-white shadow-lg sticky top-0 z-50">
      <div className="w-full max-w-7xl mx-auto px-4 py-4 flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-4">
          ...
          {/* Mobile: show hamburger on the left instead of the logo */}
  ...
  return (
    <>
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className="p-2 rounded-md bg-white/10 border border-white/20 text-white inline-flex items-center justify-center shadow-sm"
      >
...
function MobileHamburgerLeft() {
  ...
  return (
    <>
      ...
      {open && (
        <div className="absolute left-4 mt-12 w-56 bg-white text-black rounded-md p-3 shadow-lg">
          <a href="https://bitecode.co" className="flex items-center gap-2 py-2">Home</a>
          <a href="https://buymeacoffee.com/bitecode" className="flex items-center gap-2 py-2">Support</a>
        </div>
      )}
