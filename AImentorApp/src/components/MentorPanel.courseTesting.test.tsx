...
describe('mentor test session integration', () => {
  it('uses text-only ElevenLabs with lesson context and exposes actual SDK replies', async () => {
    ...
    vi.mocked(Conversation.startSession).mockImplementation(async (input: any) => {
      options = input; input.onConnect(); return { sendUserMessage, endSession: vi.fn() } as any;
    });
    ...
    await waitFor(() => expect(Conversation.startSession).toHaveBeenCalled());
    expect(options.textOnly).toBe(true);
    expect(options.overrides.conversation.textOnly).toBe(true);
